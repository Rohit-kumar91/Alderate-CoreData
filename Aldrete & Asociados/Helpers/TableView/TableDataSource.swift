...
enum ReloadType {
  ...
  case AddRowAt(indexPaths: [IndexPath], animation: UITableView.RowAnimation)
}
...
var items = Array<T>()
  ...
  private lazy var refreshControl: UIRefreshControl = {
    ...
    refreshControl.tintColor = Colors.APP_THEME.uiColor
...
//        self?.isReloadingFinished = true
//      })
    case .AddRowAt(let indexPaths, let animation):
        tableView?.beginUpdates()
        tableView?.deleteRows(at: indexPaths, with: animation)
        tableView?.endUpdates()
    }
